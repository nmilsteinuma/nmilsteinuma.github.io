...
medieval_wars_1200s <- read_excel("~/Desktop/Spring 2022/Networks/medieval_wars_1200s.xlsx")

medieval_wars_1200s.ig<-graph_from_data_frame(medieval_wars_1200s)

clp_wars_1200s <- cluster_label_prop(medieval_wars_1200s.ig)

plot(clp_wars_1200s, medieval_wars_1200s.ig, vertex.size=2,  edge.arrow.size=.1, vertex.color="gold", vertex.size=20, vertex.frame.color="gray", vertex.label.color="black", vertex.label.cex=0.3, vertex.label.dist=2, edge.curved=0.2)
...
transitivity(wars_in_medieval_3.ig, type="global")

transitivity(medieval_wars_1200s.ig, type="global")
...
graph.density(medieval_wars_1200s.ig, loops=TRUE)
...
head(medieval.nodes[order(-medieval.nodes$degree),])
...
medieval_network_1000s <- asNetwork(wars_in_medieval_2.ig)

set.seed(777)
ggplot(medieval_network_1000s, aes(x = x, y = y, xend = xend, yend = yend)) +
...
medieval_network_1100s <- asNetwork(wars_in_medieval_3.ig)

set.seed(777)
ggplot(medieval_network_1100s, aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_edges(color = "grey25", alpha = 0.25) +
  geom_nodes(color = "gold", alpha = 0.5, size = 2) +
...
medieval_network_1200s <- asNetwork(medieval_wars_1200s.ig)

set.seed(777)
ggplot(medieval_network_1200s, aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_edges(color = "grey25", alpha = 0.25) +
  geom_nodes(color = "gold", alpha = 0.5, size = 2) +
  geom_nodetext(aes( label = vertex.names ), size=1)+
  theme_blank()
