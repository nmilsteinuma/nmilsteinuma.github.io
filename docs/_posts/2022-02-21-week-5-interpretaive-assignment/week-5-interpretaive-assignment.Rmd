...
pls_work_2<-as.network(wars_in_1100s_edgelist, matrix.type = "edgelist", directed = FALSE,  hyper = FALSE, loops = FALSE, multiple = FALSE, bipartite = FALSE, vertex.attrnames=wars_in_1100s_network%v%"vertex.names")

flomarr.se_2<-equiv.clust(pls_work_2, equiv.fun="sedist", method="hamming", mode="graph")
plot(flomarr.se_2, labels=flomarr.se_2$glabels,  cex=0.3)
rect.hclust(flomarr.se_2$cluster, h=5)

```

```{r}
#blockmodel and select partitions
blk_mod<-blockmodel(pls_work_2,flomarr.se_2,k=5)
#assign block membership to vertex attribute
V(wars_in_1100s.ig)$role<-blk_mod$block.membership[match(V(wars_in_1100s.ig)$name,blk_mod$plabels)]
wars_in_1100s_network%v%"role"<-blk_mod$block.membership[match(wars_in_1100s_network%v%"vertex.names", blk_mod$glabels)]
...
blk_mod<-blockmodel(pls_work_2, flomarr.se_2, k=5)
#assign block membership to vertex attribute
pls_work_2%v%"role"<-blk_mod$block.membership[match(pls_work_2%v%"vertex.names", blk_mod$glabels)]
#plot network using "role" to color nodes: statnet
...
pls_work_3<-as.network(wars_in_1200s_edgelist, matrix.type = "edgelist", directed = FALSE,  hyper = FALSE, loops = FALSE, multiple = FALSE, bipartite = FALSE, vertex.attrnames=wars_in_1200s_network%v%"vertex.names")

flomarr.se_3<-equiv.clust(pls_work_3, equiv.fun="sedist", method="hamming", mode="graph")
plot(flomarr.se_3, labels=flomarr.se_3$glabels,  cex=0.3)
rect.hclust(flomarr.se_3$cluster, h=5)

```

```{r}
#blockmodel and select partitions
blk_mod<-blockmodel(pls_work_3,flomarr.se_3,k=5)
#assign block membership to vertex attribute
V(wars_in_1200s.ig)$role<-blk_mod$block.membership[match(V(wars_in_1200s.ig)$name,blk_mod$plabels)]
wars_in_1200s_network%v%"role"<-blk_mod$block.membership[match(wars_in_1200s_network%v%"vertex.names", blk_mod$glabels)]
...
set.seed(2)
#blockmodel and select partitions
blk_mod<-blockmodel(pls_work_3, flomarr.se_2, k=5)
#assign block membership to vertex attribute
pls_work_3%v%"role"<-blk_mod$block.membership[match(pls_work_3%v%"vertex.names", blk_mod$glabels)]
#plot network using "role" to color nodes: statnet
